...
import { getGolf } from "../../../core/util/http/golf/apiReserve";
...
export default function StaticDualCalendars({
  ...
  const handleButtonClick = (golfName, index, user) => {
    setActiveButton((nextActiveButton) =>
      nextActiveButton === golfName ? null : golfName
    );
  ...
  const handleDateChange = (newDate) => {
    ...
    if (newDate >= today && newDate < twoWeeksLater) {
      ...
      setActivePicker(
        newDate.getMonth() === today.getMonth() ? "left" : "right"
      );
      const date =
        newDate.getFullYear() +
        "-" +
