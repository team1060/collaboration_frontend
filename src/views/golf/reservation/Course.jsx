...
import { getCourse } from "../../../core/util/http/golf/apiReserve";
import { postGolf } from "../../../core/util/http/golf/apiReserve";
...
const Course = ({ golf, index, view, user }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const golfStatus = courseData.filter(
          (course) =>
            course.golf_status === 0 &&
            course.golf_date === view &&
            course.golf_no === index
        );
  ...
  const handleButtonClick = async (course) => {
    try {
      ...
      const cancelMonth = (cancelDate.getMonth() + 1)
        .toString()
        .padStart(2, "0");
