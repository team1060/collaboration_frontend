...
import { getProductInner } from "../../core/util/http/shop/apiProduct";
...
function ProductInner() {
  ...
  const handleChange = (event) => {
    ...
    const selectedOption = prop?.product_options.find(
      (option) => option.option_no === value
    );
    ...
    const fetchGolfInfo = async () => {
      try {
        ...
        const filteredResult = post.map((item) => ({
          ...item,
          image_details: Array.isArray(item.image_details)
            ? item.image_details.filter((img) => img !== null)
            : [],
          image_prdts: Array.isArray(item.image_prdts)
            ? item.image_prdts.filter((img) => img !== null)
            : [],
        }));
  ...
  const calculateTotalPrice = () => {
    ...
    for (const option of prop?.product_options || []) {
      // 각 옵션에 대한 수량 * 가격 누적
      totalPrice +=
        (selectedOption[option.option_no]?.quantity || 0) * discountedPrice;
    }
  ...
  const handleQuantityChange = (optionNo, change) => {
    ...
    const newQuantity = Math.max(
      (selectedOption[optionNo]?.quantity || 0) + change,
      0
    );
  ...
  function redirectToPayment() {
    ...
    optionInnerElements.forEach(function (optionInnerElement) {
      ...
      var image = document
        ...
        .getAttribute("src")
        .trim();
      var option = optionInnerElement
        .querySelector(".optionInnerTit")
        .innerText.trim();
      var count = optionInnerElement
        ...
        <Grid>
          <Container className="mainContainer">
            <div className="pickup">
              {prop?.is_shop_delivery ? (
                <StyledDelivery>배달가능</StyledDelivery>
              ) : (
                ""
              )}
              {prop?.is_shop_pickup ? (
                <StyledPickup>픽업가능</StyledPickup>
              ) : (
                ""
              )}
            </div>
            ...
            <Grid container style={{ justifyContent: "center" }}>
              ...
              <Grid container direction="column" item md={6}>
                ...
                <Grid className="productInfo" container>
                  ...
                  {originalPrice !== discountedPrice && (
                    <Grid
                      className="productInfoInner discountedPriceWrap"
                      container
                    >
                    ...
                    <FormControl fullWidth>
                      ...
                      >
                        {prop?.product_options?.map((option) => (
                          ...
                          >
                            {[option.option_count === 0 ? "[품절]" : ""]}
                            {option.option_name}({option.option_count}개)
                          </MenuItem>
                        ))}
                      </Select>
                    </FormControl>
                          ...
                          <Grid container justifyContent="space-between">
                            ...
                            <Grid item>
                              <Button
                                color="primary"
                                onClick={() =>
                                  handleRemoveOption(option.option_no)
                                }
                              >
                                <Clear></Clear>
                              </Button>
                            ...
                            <Grid item>
                              <Typography variant="body2">
                                <Button
                                  variant="outlined"
                                  onClick={() =>
                                    handleQuantityChange(option.option_no, -1)
                                  }
                                >
                                  <Remove></Remove>
                                </Button>
                                <Button
                                  ...
                                  variant="disabled"
                                >
                                  {selectedOption[option.option_no]?.quantity}
                                </Button>
                                <Button
                                  variant="outlined"
                                  onClick={() =>
                ...
                <Grid className="" container>
                  <Grid className="productInnerButton" item xs={6}>
                    <Button
                      size="large"
                      variant="contained"
                      color="secondary"
