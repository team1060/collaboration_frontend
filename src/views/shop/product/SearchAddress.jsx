...
const SearchAddress = ({ closeAddressModal, onSelectAddress, onSearchComplete }) => {
    ...
    const handleAddressSearch = async () => {
        try {
            ...
            setTotalPages(Math.ceil(response.data.results.common.totalCount / countPerPage));
        } catch (error) {
    ...
    const handleDetailAddressSubmit = () => {
        // 여기에서 detailAddress 정보를 활용하는 로직 추가
        // 예: API 호출, 상태 업데이트 등
    ...
    return (
        <Container className='addressFormWrap' id="SearchAddress">
                        ...
                        {addressList?.map((address, index) => (
                            <tr key={address.bdMgtSn} onClick={() => handleAddressSelect(address)}>
                                ...
                                <td className='roadAddresstbody'>
                                    <strong>{address.roadAddrPart1}</strong><br />
                                    [지번] {address.jibunAddr}<br />
                                </td>
                                <td>{address.zipNo}</td>
                            </tr>
                        ))}
            ...
            <div className="pagination">
                {searchButtonClicked && (
                    <>
                        <Button variant='outlined' onClick={handleFirstPage}>{"<<"}</Button>
                        <Button variant='outlined' onClick={handlePrevPage}>{"<"}</Button>
                        {renderPageNumbers()}
                        <Button variant='outlined' onClick={handleNextPage}>{">"}</Button>
                        <Button variant='outlined' onClick={handleLastPage}>{">>"}</Button>
                    </>
                )}
            </div>
