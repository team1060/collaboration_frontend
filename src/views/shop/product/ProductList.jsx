...
import {
  getBrand,
  getProductList,
} from "../../../core/util/http/shop/apiProduct";
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // 페이징
        const startIndex =
          (page - 1) * (isSmallScreen ? ITEMS_PAGE_SMALL : ITEMS_PAGE_LARGE);
        const endIndex =
          startIndex + (isSmallScreen ? ITEMS_PAGE_SMALL : ITEMS_PAGE_LARGE);
        ...
        <Grid container>
            ...
            <div className="all">
              ...
              <div className="but">
                <FormControl sx={{ m: 1, width: 300 }} size="small">
                  ...
                  >
                    {brandData.map((brand) => (
                      <MenuItem key={brand.brand_no} value={brand.brand_name}>
                        {brand.brand_name}
                      </MenuItem>
                    ))}
                  </Select>
          ...
          {data.map((list) => (
            <Grid item key={list.product_no}>
              <Card
                sx={{ width: { lg: 230, xs: 225 }, height: 420 }}
                className="card"
              >
                <CardContent>
                  ...
                  >
                    {/* 가격 */}
                    {`${Number(
                      list.price * (1 - list.discount)
                    ).toLocaleString()}원`}
                    {/* 할인 */}
                    <span>
                      {`${
                        list.discount > 0
                          ? `[${(list.discount * 100).toFixed()}%]`
                          : ""
                      }`}
                    </span>
                  </Typography>
        ...
        <div className="page">
          <Stack spacing={2}>
            <Pagination
              ...
              count={
                isSmallScreen
                  ? Math.ceil(productLists.length / ITEMS_PAGE_SMALL)
                  : Math.ceil(productLists.length / ITEMS_PAGE_LARGE)
              }
              color="primary"
