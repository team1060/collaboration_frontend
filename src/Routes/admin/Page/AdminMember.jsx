...
const AdminMember = () => {
  ...
  const fetchMembers = async () => {
    ...
    // 날짜 순으로 정렬
    const sortedData = data.sort((a, b) => new Date(b.regdate) - new Date(a.regdate));
  ...
  const columns = [
    ...
    { field: 'type', headerName: '등급', width: 100, valueGetter: (params) => ["회원", "중간관리자", "관리자"][params.row.type] },
 ...
 const filteredMembers = members.filter(member => {
  ...
  if (filterCategory === 'type') {
    ...
    return typeNames[member.type].toLowerCase().includes(searchTerm.toLowerCase());
  } else {
    const memberValue = member[filterCategory] ? member[filterCategory].toString() : '';
...
const handleUpdateClick = () => {
  if (selectedRowIds.length === 1) {
    const selectedRowData = members.find(member => member.id === selectedRowIds[0]);
